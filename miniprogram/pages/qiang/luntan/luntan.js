...
Page({
    ...
    loadLuntan() {
        ...
        wx.cloud.database().collection('luntan')
            .where({
                isHege: true,
            })
            ...
            .then(res => {
                console.log('下载的订单列表 luntanList 为：', res.data)
    ...
    onReachBottom: function () {
        ...
        var luntanList = this.data.luntanList
        var pageNum = this.data.pageNum + 1
        console.log('页面触底');
        if (this.data.isPinche) {
            wx.cloud.database().collection('luntan').where({
                    isHege: true,
                })
                .orderBy('isXiajia', 'asc')
                .orderBy('isZhiding', 'desc')
                ...
                .then(res => {
                    if (res.data.length == 0) {
                        ...
                        res.data.forEach(element => {
                            luntanList.push(element)
                        });
                        console.log('当前下载luntanList第' + pageNum + '页：', luntanList)
                        this.setData({
                            luntanList,
                            pageNum,
                        })
